...
def Ni_crab(industry_new):
    
    
    # 要抓取的網址
    url = 'https://goodinfo.tw/StockInfo/StockList.asp?MARKET_CAT=全部&INDUSTRY_CAT='+industry_new+'&SHEET=年獲利能力_近N年一覽&SHEET2=稅後淨利年成長率(%25)&RPT_TIME=最新資料'
    ...
    stockNi=soup.find_all(id=re.compile('^row'))
    ...
    for Ni in range(0,len(stockNi)):
        stocknum=stockNi[Ni].find_all('nobr')[0].text
        resultNi=stockNi[Ni].find_all('nobr')[-1].text
        stock_num_list.append(stocknum)
        stock_Ni_list.append(resultNi)
    
    
    
    #欄位合併
    stockresult_list={"代號":stock_num_list,"稅後淨利年成長率(%)":stock_Ni_list}
    df_stock=pd.DataFrame(stockresult_list)
    
    
    df_stock=df_stock[df_stock['稅後淨利年成長率(%)'].str.contains("\+")]
    ...
    #params = {"message": industry_new+'相關類股其今年累計稅後淨利年度成長率優於去年者，適合購買的股票為:'+'\n'+result_stock}
