...
def Oi_crab(industry_new):
    
    
    # 要抓取的網址
    url = 'https://goodinfo.tw/StockInfo/StockList.asp?MARKET_CAT=全部&INDUSTRY_CAT='+industry_new+'&SHEET=年獲利能力_近N年一覽&SHEET2=營業收入年成長率(%25)&RPT_TIME=最新資料'
    ...
    stockOi=soup.find_all(id=re.compile('^row'))
    ...
    for Oi in range(0,len(stockOi)):
        stocknum=stockOi[Oi].find_all('nobr')[0].text
        resultOi=stockOi[Oi].find_all('nobr')[-1].text
        stock_num_list.append(stocknum)
        stock_Oi_list.append(resultOi)
    
    
    
    #欄位合併
    stockresult_list={"代號":stock_num_list,"營業收入年成長率(%)":stock_Oi_list}
    df_stock=pd.DataFrame(stockresult_list)
    
    
    df_stock=df_stock[df_stock['營業收入年成長率(%)'].str.contains("\+")]
    ...
    for i in df_stock_num_result:
        ...
        #print(industry_new+'相關類股其今年累計營業收入年度成長率優於去年者，適合購買的股票為:'+'\n'+result_stock)
    """
    ...
    #params = {"message": industry_new+'相關類股其今年累計營業收入年度成長率優於去年者，適合購買的股票為:'+'\n'+result_stock}
